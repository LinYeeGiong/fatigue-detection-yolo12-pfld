...
$('#image-submit')?.addEventListener('click',async()=>{if(!images.files.length)return toast('请先选择图片');const button=$('#image-submit');button.disabled=true;button.textContent='检测中…';const data=new FormData();[...images.files].forEach(file=>data.append('files',file));try{const body=await jsonRequest('/api/detect/images',{method:'POST',body:data});$('#image-empty').hidden=true;$('#image-results').innerHTML=body.results.map((item,index)=>`<article class="result-card"><img src="${item.processed_image||URL.createObjectURL(images.files[index])}" alt="${item.filename} 检测结果"><div><span class="level ${item.level}">${labels[item.level]}</span><h3>${item.filename}</h3><p><span>疲劳分值</span><strong>${item.score}</strong></p><p><span>EAR / MAR</span><strong>${item.metrics.ear.toFixed(2)} / ${item.metrics.mar.toFixed(2)}</strong></p></div></article>`).join('');if(body.alert)showAlert()}catch(error){toast(error.message)}finally{button.disabled=false;button.textContent='开始检测'}});
